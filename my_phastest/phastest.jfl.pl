...
my $num = $ARGV[1];
my $mode = $ARGV[2];
my $anno_flag = $ARGV[3];	# 0 if prophage region only, 1 for annotating all proteins in genome.
my $single_diamond_flag = $ARGV[4]; # 0 if using multiple diamond processes, 1 if single.
...
# Parameters for running on cluster.
#
# Note: During parameter optimization tests in Jan 2016, on the cluster with 112 cores
# (botha-w1 to botha-w11), it was found that the following values gave the fastest
# runtimes on average when all nodes were available, with the target databases split
# into 8 parts:
#
# BLAST against prophage_virus.db:
#   use_split_db_vir = 0; # do not split up the viral DB
#   target_query_pieces: 104
#   cores_per_blast_job: 1
#
# BLAST against bacteria_all_select.db:
#   use_split_db_bac = 1; # split up the bacterial DB
#   target_query_pieces: 6
#   cores_per_blast_job: 2
#
# As of 2022, Botha3 cluster supports up to 216 CPU available for processing.

my $use_split_db_bac = 0; # split up the bacterial DB
my $target_query_pieces_bac = 8;
my $cores_per_blast_job_bac = 1;
...
if ($mode eq "-d") {
	...
	$use_split_db_bac = 1; # split up the bacterial DB
	$target_query_pieces_bac = 1;
	$cores_per_blast_job_bac = 1;
}

if ($single_diamond_flag == 1) {
	$use_split_db_bac = 0;
	$target_query_pieces_bac = 1;
	$cores_per_blast_job_bac = 1;
}
...
my $use_split_db_vir = 0; # do not split up the viral DB
my $target_query_pieces_vir = 208;
my $cores_per_blast_job_vir = 1;
...
# my $phage_finder_tolerate_time=180*60 ; #sec
# my $tRNAscan_tolerate_time=60*60 ; #sec
...
# If file extension is in .fasta, change to .fna
if (-e "$jobs_dir/$num/$num.fasta") {
	system("mv $jobs_dir/$num/$num.fasta $jobs_dir/$num/$num.fna");
}
...
system("echo 'Job $num is running. $running_case_file is generated!' >>$log_file" );
...
if ($flag eq '-s' or $flag eq '-c'){ # have fasta seq or contigs file, so use prodigal or FragGeneScan to predict ORF and create .ppt .fna .faa files;
	...
	my $postfix = '';
	if ($flag eq '-c') {
		# genrate $num\_contig_positions.txt
		...
		if (!-s "$num$postfix.fna"){
			system("cp $num\_original.fna $num$postfix.fna");
		}
	}
	elsif ($flag eq '-s') {
	...
	if ($flag eq '-s') {
		...
		# Call FragGeneScan on contigs file
		...
		my $fraggenescan_dir = "$jobs_dir/$num/tmp/fraggenescan";
		if (not -x "$scripts_dir/call_fraggenescan_parallel.sh") {
			system("chmod u+x $scripts_dir/call_fraggenescan_parallel.sh"); # make script executable if needed
		}
		my $cm = "$scripts_dir/call_fraggenescan_parallel.sh $fraggenescan_dir $num $log_file";
		system("echo '$cm' >> $log_file");
		my $ret = system("$cm");
		if ($ret != 0) {
			if ($ret == -1) {
				my $msg = "call_fraggenescan_parallel.sh failed to run";
				open (LOG, ">>$log_file");
				print LOG "$msg\n";
				LOG->flush();
				close(LOG);
				...
				my $exitcode = $ret >> 8;
				my $msg = "call_fraggenescan_parallel.sh failed with exit status $exitcode";
				open (LOG, ">>$log_file");
				print LOG "$msg\n";
				LOG->flush();
				close(LOG);
	...
	my $cm = "perl $scripts_dir/change_to_ptt_format.pl $num.fna $num.predict  $num.ptt";
	system("echo '$cm' >> $log_file");
	system("$cm");
...
for(my $i=1; $i<=2; $i++){
	...
	if (not defined $pid) {
		...
		if ($i ==1){
			...
			my $cmd = "$scripts_dir/phage_finder.sh $num $cluster_head_virus_database $cluster_child_virus_database $use_split_db_vir $target_query_pieces_vir $cores_per_blast_job_vir $queue $log_file\.2";
			# print LOG2 $cmd	. "\n";
			# LOG2->flush();
			# close(LOG2);
			...
			if ($ret != 0) {
				if ($ret == -1) {
					...
					print "phage_finder.sh failed to run\n";
					# LOG2->flush();
					# close(LOG2);
					...
					my $exitcode = $ret >> 8;
					print "phage_finder.sh failed with exit status $exitcode\n";
...
if ($flag ne '-g' && !-e "$jobs_dir/$num/ncbi.out.non_hit_pro_region"){
	...
	if (-z "$jobs_dir/$num/$num.faa.non_hit_pro_region") {
		...
		system("echo 'skip call_remote_dmnd.sh and touch ncbi.out.non_hit_pro_region took $time sec' >> $log_file");
	} else {
		...
		my $start_call_remote_dmnd_time = time;
		my $cm = "$scripts_dir/call_remote_dmnd.sh  $blast_b_dir  $jobs_dir/$num/$num.faa.non_hit_pro_region  $cluster_head_bac_database $cluster_child_bac_database $jobs_dir/$num/ncbi.out.non_hit_pro_region $use_split_db_bac $target_query_pieces_bac $cores_per_blast_job_bac $log_file";
		if ($single_diamond_flag == 1) {
			$cm = "$scripts_dir/call_remote_dmnd_single.sh  $blast_b_dir  $jobs_dir/$num/$num.faa.non_hit_pro_region  $cluster_head_bac_database $cluster_child_bac_database $jobs_dir/$num/ncbi.out.non_hit_pro_region $log_file";
		}
		open (LOG, ">>$log_file");
		print LOG "$cm\n";
		LOG->flush();
...
sub cleanup{
	...
	my $cur_dir = getcwd;
	chdir "$jobs_dir/$num";
 	my $cmd = "rm -rf *_dir  extract_RNA_result*  ncbi* tmRNA* tRNAscan* true_defective_prophage.txt *faa *non_hit_pro_region *predict *process *ptt *gbk *txt_* *.txt.old image.png *tmp *fai";
...
sub generate_timing_message{
	my $num = shift;
	my $log = "$jobs_dir/$num/$num.log";
	my @arr = ();

	open('log') or die("Could not open log file.");
	foreach my $line (<log>) { #<log_file>
		chomp($line);
...
sub tRNA_tmRNA{
    my ($num, $log_file)=@_;
	# open (LOG, ">>$log_file");
	print "tRNAscan-SE -B -o tRNAscan.out $num.fna\n";
	print "find tRNA sequences ...\n";
    
    my $trnascan_start = time;
	print "running: /storage/Documents/service/externe/lcfortier/programs/phastest/slurm-docker-cluster/phastest-app-docker/sub_programs/tRNAscan-SE-1.23/bin/tRNAscan-SE -B -o tRNAscan.out $num.fna 2>&1\n";
	my $pid = open(my $fh, '-|', "/storage/Documents/service/externe/lcfortier/programs/phastest/slurm-docker-cluster/phastest-app-docker/sub_programs/tRNAscan-SE-1.23/bin/tRNAscan-SE -B -o tRNAscan.out $num.fna 2>&1") or die $!;
	while (my $line = <$fh>) {
		print $line;
		if ($line =~ /WARNING: tRNAscan.out exists already/) {
			...
			# print "tRNAscan-SE found already existing tRNAscan.out file! Kill tRNAscan-SE and abort!\n";
			# LOG->flush();
			# close(LOG);
			exit(-1);
		}
	...
	if ($ret != 0) {
		if ($ret == -1) {
			print "tRNAscan-SE failed to run\n";
			# LOG->flush();
			# close(LOG);
			exit(-1); # child process fails and exits
		} else {
			my $exitcode = $ret >> 8;
			print "tRNAscan-SE failed with exit status $exitcode\n";
			# LOG->flush();
			# close(LOG);
			exit(-1); # child process fails and exits
		}
    ...
    if ($ret != 0) {
    	if ($ret == -1) {
				print  "extract_RNA.pl failed to run\n";
				# LOG->flush();
				# close(LOG);
				exit(-1); # child process fails and exits
    	} else {
    		my $exitcode = $ret >> 8;
				print "extract_RNA.pl failed with exit status $exitcode\n";
				# LOG->flush();
				# close(LOG);
    		exit(-1); # child process fails and exits
    	}
	...
	print "running make_RNA_png_input\n";
    ...
    if ($ret != 0) {
    	if ($ret == -1) {
				print "make_RNA_png_input.pl failed to run\n";
				# LOG->flush();
				# close(LOG);
				exit(-1); # child process fails and exits
    	} else {
    		my $exitcode = $ret >> 8;
				print  "make_RNA_png_input.pl failed with exit status $exitcode\n";
				# LOG->flush();
				# close(LOG);
    		exit(-1); # child process fails and exits
    	}
